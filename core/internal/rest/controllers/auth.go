...
func (router *Router) SignIn(ctx *gin.Context) {
	...
	if err != nil {
		router.newErrorResponse(ctx, repository.ErrorResponse{Status: http.StatusInternalServerError, Message: err.Error()})
		return
	}
...
func (router *Router) SignOut(ctx *gin.Context) {
	...
	if len(header["Cookie"]) == 0 {
		ctx.JSON(401, repository.ErrorResponse{Status: http.StatusUnauthorized, Message: "User is unauthorized"})
		ctx.Abort()
		return
	}
...
func (router *Router) UserValidation(ctx *gin.Context) {
	...
	if err.IsError() {
		ctx.JSON(401, repository.ErrorResponse{Status: 401, Message: "User is unauthorized"})
		ctx.Abort()
		return
	}
